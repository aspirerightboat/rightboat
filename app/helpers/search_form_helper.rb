module SearchFormHelper
  def manufacturers_picker_field(name, manufacturer_ids = nil, field_id = nil)
    ...
    manufacturers = (Manufacturer.where(id: manufacturer_ids).order(:name).pluck_h(:id, :name) if manufacturer_ids)
  ...
  def models_picker_field(name, model_ids = nil, field_id = nil)
    ...
    models = (Model.where(id: model_ids).order(:name).pluck_h(:id, :name) if model_ids)
