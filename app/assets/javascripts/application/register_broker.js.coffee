$ ->
  $('#register_broker_form').each ->
    ...
    $broker_form.on 'ajax:success', (e, data) ->
      ...
      $('.card-fields-number input').payment('formatCardNumber')
      $('.card-fields-expire input').payment('formatCardExpiry')
      $('.card-fields-cvc input').payment('formatCardCVC')
      $card_form.closest('.em-section').hide().slideDown(200)
      ...
      $card_form.submit (event) ->
        ...
        Stripe.card.createToken $card_form, (status, response) ->
          if response.error
            ...
            $.post('/register-broker/add_card', params)
            .done (data) ->
              $card_form.closest('.em-section').slideUp(200)
