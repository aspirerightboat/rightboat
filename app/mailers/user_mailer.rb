class UserMailer < ApplicationMailer
  ...
  def boat_status_changed(user_id, boat_id, reason, alert_reason)
    ...
    if @reason == 'deleted'
      @other_boats = Rightboat::BoatSearch.new.do_search(manufacturer_model: @boat.manufacturer_model, includes: [:user, :currency, :manufacturer, :model, :primary_image, :vat_rate, :country]).results
